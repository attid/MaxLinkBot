...
MaxLinkBot это персональный многопользовательский шлюз между Telegram и MAX через Pumax. Один бот обслуживает нескольких разрешенных Telegram-пользователей, но каждый пользователь работает только со своей собственной MAX-сессией и своими собственными чатами.
...
- `infrastructure/`:
  aiogram, `maxapi`, Pumax, SQLite, Docker-конфигурация, адаптеры.
- `interface/`:
