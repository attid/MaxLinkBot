...
После завершения — план переносится из `docs/exec-plans/active/` в `docs/exec-plans/completed/` с отметками о выполнении. Если работа уже готова к финальному коммиту, перенос плана должен быть сделан до этого коммита и входить в тот же финальный diff.
...
7. Если все шаги плана выполнены — перенести plan из `docs/exec-plans/active/` в `docs/exec-plans/completed/` до финального коммита и включить этот перенос в финальный коммит.
8. Написать подробный коммит-месседж: **Что** сделано? **Почему**? **Как** это было протестировано?
9. Если задача затрагивает публичные контракты (API, схемы, events) — обновить соответствующие спецификации.
